...
$('#citybtn1').on("click", function(event) {
    ...
    $.ajax({
        ...
        console.log(response)
            ...
            var cityH1 = $('<h1>').text(response.name)
            console.log(response.name)
            var tempLi = $('<li>').text("Temperature: " + response.main.temp)
            console.log(response.main.temp)
            var feels_likeLi = $('<li>').text("Feels like: " + response.main.feels_like)
            console.log(response.main.feels_like)
            var humidityLi = $('<li>').text("Humidity: " + response.main.humidity)
            console.log(response.main.humidity)
            var windspeedLi = $('<li>').text("Wind Speed: " + response.wind.speed)
